...
// injected imports
import '/../docs/pages/template-list/template-list.demo.ts';
import '@roenlie/mirage-docs/app/components/page/page-header.js';
// hoisted


class xbxvgnpquv extends PageAdapter {
	...
	<midoc-page-header
		component-name="mm-template-list"
		.declaration=${this.metadata['mm-template-list']}
	></midoc-page-header>
</div>
<p><mm-template-list-demo></mm-template-list-demo></p>
...
const module = new ContainerModule(({rebind}) => {
	rebind('midoc-page').toConstantValue(xbxvgnpquv);
});
