...
export interface ProgressbarOptions {
	 ...
	 *
	 * set to TRUE: Assume that eah of the remaining steps will take as long as THE LAST STEP took.
	 * 	WARNING: This implies, that every call of the ProgressBar.update() function increment the state with the same stepwidth.
...
export class ConsoleBar {
	...
	*/
	...
	constructor(options?: string | ProgressbarOptions) {
		...
		for (const opt in options) {
			if (this[opt as keyof this] !== undefined)
				this[opt as keyof this] = options[opt as keyof ProgressbarOptions] as any;
		}
	...
	renderLine(): string {
		...
		const countString = `[${ currentString }/${ this.total }]`;
		...
		const overAllString = this.formatTime(this.overallTime);
		const elapsedString = this.formatTime(this.elapsed);
		const ttfString = this.formatTime(this.timeToFinish);

		// Replace macros
		let line = this.formatString
			.replace(/#bar/g, barString)
			.replace(/#count/g, countString)
			.replace(/#percent/g, percentString)
			.replace(/#overall/g, overAllString)
			.replace(/#elapsed/g, elapsedString)
			.replace(/#ttf/g, ttfString)
			.replace(/#message/g, this.message)
			.replace(/#spinner/g, this.currentSpinnerSymbol);

		// Colors
		line = line
			.replace(/##default/g, colorCodes.Reset)
			.replace(/##green/g, colorCodes.Green)
			.replace(/##blue/g, colorCodes.Blue)
			.replace(/##red/g, colorCodes.Red)
			.replace(/##yellow/g, colorCodes.Yellow)
			.replace(/##bright/g, colorCodes.Bright)
			.replace(/##dim/g, colorCodes.Dim);
	...
	update(current: number, message?: string): this {
		...
		const timePerStep = this.lastUpdateForTiming ? (now - this.lastUpdate) : (this.elapsed / this.#current);
	...
	private formatTime(millis: number): string {
		...
		const hours = Math.round(millis % (1000 * 60 * 60 * 24) / (1000 * 60 * 60));
		const minutes = Math.round(millis % (1000 * 60 * 60) / (1000 * 60));
