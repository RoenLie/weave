...
import { COLORS } from '../constants/board.constant.js';
...
describe('Should properly calculate possible moves', () => {
	...
	it('White can do a en Passant from FEN (#11)', () => {
		...
		const game = new Game('rnbqkbnr/1pp2pp1/p3p2p/3pP3/3P4/P7/1PP2PPP/RNBQKBNR w KQkq d6 0 1');

		//console.log(game.exportJson());
		//console.log(game.printToConsole());
		//console.log('I MOVE WHWAT', game.moves('E5'));
...
describe('Should properly calculate score by pieces locations', () => {
	...
	it('for pawns', (ctx) => {
		...
		expect(game.board.calculateScoreByPiecesLocation()).to.be.equal(0.5 * ctx.multiplier);
	});
});
