...
import { COLORS, COLUMNS, PIECE_VALUES, PIECES, ROWS } from './constants/board.constant.js';
import { BoardConfiguration, BoardLocation, Piece, PieceColor } from './types.js';


export const printToConsole = (configuration: BoardConfiguration) => {
	process.stdout.write('\n');
	let fieldColor: PieceColor = COLORS.WHITE;
...
export const getFEN = (configuration: BoardConfiguration) => {
	let fen = '';
	Object.assign([], ROWS).reverse().map(row => {
		let emptyFields = 0;
		if (row < 8)
...
export const getJSONfromFEN = (fen = '') => {
...
export const isPieceValid = (piece: Piece) =>
	Object.values(PIECES).includes(piece);


export const getPieceValue = (piece: Piece) =>
	PIECE_VALUES[piece.toUpperCase() as keyof typeof PIECE_VALUES] || 0;
